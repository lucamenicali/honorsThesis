...
var
    ...
    f_ttt, pi_star, z1t, z2t, rt, c_f_start, lt, tau_ct, st;
...
parameters
    ...
    pi_ss, phi_pi, phi_y, sigma_iota, gamm_gl, gamm_gy, l_ss, y_h_ss, c_f_star
    
    gamm_cl, gamm_cy, gamm_kl, gamm_ky, gamm_nl, gamm_ny, lambd_h, lambd_i, lambd_y, pi_h_ss
...
// 3. Intratemporal Euler equation for wages (424)
    chi_n * (exp(nt)^(eta)) = ( 1 - tau_n ) * exp(wt) * (exp(ct)^(-sigma)) * ( 1 / ( 1 + exp(tau_ct) ) );

// 4. Intertemporal Euler equation for money holdings (423)
    chi_m * (exp(mt)^(-muu)) = (exp(ct)^(-sigma)) * (1 / ( 1 + exp(tau_ct) )) - bet * (exp(ct(+1))^(-sigma)) * ( 1 / (1 + exp(tau_ct(+1)) )) * (1 / exp(pit(+1)));
...
// 11. Production constraint (434);
    exp(y_ht) = exp(c_ht) + exp(xt) + ( s_g * exp(y_ht) ) + exp(c_f_start);
...
// 14. Terms of trade (438)
    exp(ttt) / exp(ttt(-1)) = (exp(st) / exp(st(-1))) * pi_h_st / exp(pi_ht);
...
// 18. z1 (442)
    exp(z1t) = ( exp(thetat)^(1 - fi) ) * exp(y_ht) * ( exp(ttt)^(nuu-1) ) + bet * theta * ( (exp(ct(+1))^(-sigma)) / (exp(ct)^(-sigma)) ) * ( (1 + exp(tau_ct)) / (1 + exp(tau_ct(+1)) )) * ( ( exp(thetat) / exp(thetat(+1)) )^(1 - fi) ) * ( ( 1 / exp(pi_ht(+1)) )^(1-fi) ) * exp(z1t(+1));

// 19. z2 (443)
    exp(z2t) = ( exp(thetat)^(-fi) ) * exp(y_ht) * exp(mct) + bet * theta * ( (exp(ct(+1))^(-sigma)) / (exp(ct)^(-sigma)) ) * ( (1 + exp(tau_ct)) / (1 + exp(tau_ct(+1)) )) * ( ( exp(thetat) / exp(thetat(+1)) )^(-fi) ) * ( ( 1 / exp(pi_ht(+1)) )^(-fi) ) * exp(z2t(+1));

// 20. Interest rate (monetary policy one) (420)
    bet * ( exp(ct(+1))^(-sigma) ) * exp(rt) * ( 1 / (1 + exp(tau_ct(+1))) ) * ( 1 / exp(pit(+1)) ) = ( 1 / (1 + exp(tau_ct)) ) * ( exp(ct)^(-sigma) );

// 21. Intertemporal equation on int. rate on foreign assets (421)
    bet * ( exp(ct(+1))^(-sigma) ) * exp(qt) * ( 1 / (1 + exp(tau_ct(+1))) ) * ( 1 / exp(pi_star(+1)) ) * ( exp(ttt(+1))^(nuu_star + nuu - 1) ) = ( 1 / (1 + exp(tau_ct)) )  * ( exp(ct)^(-sigma) ) * ( exp(ttt)^(nuu_star + nuu - 1) ) * (1 + phi_h * ( exp(f_ht) * ( exp(ttt)^(nuu_star+nuu-1) ) - f_h_ss * ( tt_ss^(nuu_star+nuu-1) ) ) );

// 22. Intertemporal E.E. with respect to capital (422)
    bet * ( exp(ct(+1))^(-sigma) ) * (exp(ttt(+1))^(nuu - 1)) * ( 1 / (1 + exp(tau_ct(+1))) ) * (1 - delt + ( 1 - tau_k ) * exp(r_kt(+1))) = (exp(ct)^(-sigma)) * (exp(ttt)^(nuu-1)) * ( 1 / (1 + exp(tau_ct)) );

// 23. Gov't budget constraint (433)
    s_b * exp(y_ht) * (exp(ttt)^(nuu-1)) + exp(mt) + (exp(ttt)^(nuu_star+nuu-1)) * exp(f_ttt) / exp(ttt)^(nuu_star) = (phi_g/2) * (( ( exp(f_ttt) / exp(ttt)^(nuu_star) ) * (exp(ttt)^(nuu_star + nuu - 1)) - ( f_tt_ss / tt_ss^(nuu_star) ) * (tt_ss^(nuu_star + nuu - 1)) )^(2)) + exp(rt(-1)) * (1 / exp(pit)) * s_b * exp(y_ht(-1)) * (exp(ttt(-1))^(nuu-1)) + (1 / exp(pit)) * exp(mt(-1)) + exp(qt(-1)) * (exp(ttt)^(nuu_star + nuu - 1)) * (1 / exp(pi_star)) * (exp(f_ttt(-1)) / (exp(ttt(-1))^(nuu_star))) + exp(ttt)^(nuu - 1) * s_g * exp(y_ht) - exp(tau_ct) * ((exp(ttt)^(nuu - 1)) * exp(c_ht) + (exp(ttt)^(nuu)) * exp(c_ft)) - tau_k * ( exp(r_kt) * ( exp(ttt)^(nuu - 1) ) * exp(kt(-1)) + exp(dt) ) - tau_n * exp(nt) * exp(wt) - s_l * exp(y_ht) * (exp(ttt)^(nuu - 1));
...
//    exp(sgt) - s_g = - gamm_gl * ( exp(lt(-1)) - l_ss ) + gamm_gy * ( exp(y_ht) - y_h_ss );

// 29. Endogenous consumption tax
    exp(tau_ct) / tau_c = ( exp(lt(-1)) / l_ss )^(gamm_cl) * ( exp(y_ht(-1)) / y_h_ss )^(gamm_cy);
...
// 34. Taylor Rule
//    log( exp(rt) / r_cap ) = phi_pi * log( exp(pit(-1)) / pi_ss ) + phi_y * log( exp(y_ht(-1)) / y_h_ss );    
       exp(rt) / r_cap = (exp(rt(-1)) / r_cap)^(phi_R) * ( ( exp(pit) / pi_ss )^(phi_pi) * ( exp(y_ht) / y_h_ss )^(phi_y) * (exp(st) / epst)^(phi_s) )^(1 - phi_R) ;  

// 35. UIP
        exp(rt) - exp(pit) = (exp(qt) - exp(pit)) * ( (exp(ct(+1))^(-sigma)) / (exp(ct)^(-sigma)) ) * ( (1 + tau_c) / (1 + tau_c )) * ( ( exp(thetat) / exp(thetat(+1)) )^(-fi) ) * ( 1 / exp(pi_ht(+1)) ) * (exp(st(+1)) / exp(st));
...
osr_params gamm_cl gamm_cy phi_R phi_s;
...
osr(order=1, irf=0, opt_algo=9) y_ht pit rt st;
