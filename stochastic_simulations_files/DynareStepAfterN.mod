...
var
    ...
    f_ttt, pi_star, z1t, z2t, rt, c_f_start, lt, tau_nt, kdy, st;
...
// 3. Intratemporal Euler equation for wages (424)
    chi_n * (exp(nt)^(eta)) = ( 1 - exp(tau_nt) ) * exp(wt) * (exp(ct)^(-sigma)) * ( 1 / ( 1 + tau_c ) );
...
// 31. Endogenous income tax
    exp(tau_nt) / tau_n = ( exp(lt(-1)) / l_ss )^(gamm_nl) * ( exp(y_ht(-1)) / y_h_ss )^(gamm_ny);
...
osr_params gamm_nl gamm_ny phi_R phi_s;
